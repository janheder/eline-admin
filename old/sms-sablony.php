...
if($_GET['action']=='update')
{
			...
			{		
					   ...
					   if($form->errors==false)
					   {
							...
							$query = Db::update('sms_sablony', $data_update, $where_update);
							    ...
							    'udalost' => 'SMS šablony - úprava záznamu id '.intval($_POST['id']),
	...
	else
	{
		if($_GET['id'])
		{
			$data = Db::queryRow('SELECT * FROM sms_sablony WHERE id = ?', array($_GET['id']));
			...
			if($data)
			{
				...
				echo $form->inputTextarea('text','text',$data['text'],'Text SMS','','',1,0,'');
...
elseif($_GET['action']=='insert')
{
	...
	if($form->submit())
	{
			if($form->errors==false)
		    {
				...
				$query_insert = Db::insert('sms_sablony', $data_insert);
		        ...
		        $data_insert_log = array(
							    'id_admin' => $_SESSION['admin']['id'],
							    'uz_jm' => $_SESSION['admin']['uz_jm'],
							    'udalost' => 'SMS šablony - přidání záznamu: '.strip_tags($_POST['nazev']),
							    'url' => sanitize($_SERVER['HTTP_REFERER']),
							    'ip' => sanitize(getip()),
							    'datum' => time()
							     );
	...
	{			
				...
				echo $form->inputTextarea('text','text','','Text SMS','','',1,0,'');
				echo $form->addHTML('<div class="clear" style="height: 20px;"></div>');	
				echo $form->addHTML('<b>Proměnné do šablon</b><br>');	
				echo $form->addHTML('[CELKOVA_CASTKA] = celková částka za objednávku<br>');	
				echo $form->addHTML('[CISLO_UCTU] = číslo účtu z obecného nastavení pro daný eshop<br>');	
...
else
{
    ...
    echo 'Registrace na SMS bráně <a href="https://www.sms-sluzba.cz/users/new?prov_user='.__SMS_AFF__.'" target="_blank">zde</a>.<br><br>';
	...
	echo '<table border="0" class="ta" cellspacing="0" cellpadding="6">
		  ...
		  <tr><td colspan="5" style="text-align: right;"><a href="./index.php?p=objednavky&pp=sms-sablony&action=insert">přidat záznam</a></td></tr>
		  <tbody class="row_position">';

	$data_s = Db::queryAll('SELECT * FROM sms_sablony WHERE 1 ORDER BY razeni ASC', array());
	if($data_s !== false ) 
	{
...
$(".row_position").sortable({
	...
	stop: function() {
		...
		updateOrder(selectedData,'razeni_sms_sablony');
	}
